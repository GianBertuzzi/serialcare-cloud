...
function AdminOrdenes() {
  ...
  async function handleDetailUpdated(message) {
  ...
  const columns = [
    ...
    { key: "tecnico", label: "Tecnico", searchValue: (orden) => orden.tecnico_nombre || "", render: (orden) => orden.tecnico_nombre || "Sin asignar" },
    { key: "tipo_atencion", label: "Tipo", searchValue: (orden) => orden.tipo_atencion || orden.tipo_orden || "", render: (orden) => orden.tipo_atencion || orden.tipo_orden },
    ...
    { key: "acciones", label: "Acciones", searchable: false, sortable: false, render: (orden) => (
      <div className="table-actions action-stack">
        <button className="btn btn-outline-primary btn-sm" type="button" onClick={() => setSelected(orden)}>Especificaciones</button>
        <select className="form-select form-select-sm state-select" value={orden.estado} disabled={updatingId === orden.id_orden} onChange={(event) => handleEstadoChange(orden.id_orden, event.target.value)}>
          {ESTADOS_ORDEN.map((estado) => <option key={estado} value={estado}>{estado}</option>)}
        </select>
      </div>
    )}
  ];
  ...
  return (
    <AppLayout title="Ordenes de servicio" eyebrow="ADMIN">
      {success ? <p className="alert alert-success">{success}</p> : null}
      {formOpen ? <NewOrderForm onCancel={() => setFormOpen(false)} onCreated={handleCreated} /> : null}
      {selected ? <OrdenDetalleModal orden={selected} onClose={() => setSelected(null)} onUpdated={handleDetailUpdated} /> : null}
      <DataTable
